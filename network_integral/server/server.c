...
int wait_for_tasks(worker_t *workers, int N, double *result_sum);
...
void send_tasks(worker_t *workers, int N)
{
    ...
    for (int i = 0; i < N; ++i)
    {
        ...
        ssize_t bytes_sent = send(worker.socket, &worker.task, sizeof(task_t), 0);
        if (bytes_sent != sizeof(task_t))
        {
    ...
    while (count < N)
    {
        ...
        for (int i = 0; i < N; i++)
        {
            ...
            else if (incoming == 2) /* done */
            {
                ...
                printf("worker number %d finished ", i);
                printf("sending THNX...\n");
